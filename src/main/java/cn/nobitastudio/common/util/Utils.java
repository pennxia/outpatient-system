...
public abstract class Utils {
	...
	public static <T> T getOne(List<T> result) {
		return isNullOrEmpty(result) ? null : result.get(0);
	}
...
//	}
	...
	public static Method getObjWriteMethod(Object obj, String propertyName) {
	...
	public static void setObjValue(Object obj, String propertyName, Object value) {
		Method idSetMethod = Utils.getObjWriteMethod(obj, propertyName);
		if (idSetMethod != null) {
			try {
				idSetMethod.invoke(obj, value);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	 ...
	 *            已经有值得对象
	 */
	public static void setSomePropValue(Object obj, Object value) {
		PropertyDescriptor[] beanProps = Utils.getBeanInfo(obj).getPropertyDescriptors();
		PropertyDescriptor[] ctxProps = Utils.getBeanInfo(value).getPropertyDescriptors();
		for (PropertyDescriptor prop : beanProps) {
			for (PropertyDescriptor ctxProp : ctxProps) {
				try {
					if (prop.getName().equals(ctxProp.getName()) && prop.getPropertyType() == ctxProp.getPropertyType()) {
						prop.getWriteMethod().invoke(obj, ctxProp.getReadMethod().invoke(value));
					}
				} catch (Exception e) {
					throw Throwables.propagate(e);
				}
	...
	/**
	 * 将一个Map转变为Bean，当map的key与bean的属性名相同时，将map对应的值赋值给bean
	 * 
	 * @param mapData
	 * @param bean
	 * @param func
	 ...
	 * @return
	 */
	public static <T> T mapToBean(Map<String, Object> mapData, Class<T> bean, Function<String, String> func) {
		T t = null;
		try {
			t = bean.newInstance();
		} catch (Exception e) {
		...
		for (Entry<String, Object> entry : mapData.entrySet()) {
			...
			if (func != null)
				key = func.apply(key);
			Method setMethod = Utils.getObjWriteMethod(t, key);
			if (setMethod == null)
				continue;
			Class<?> clazz = setMethod.getParameterTypes()[0];
			try {
				if (clazz == Integer.class || clazz == int.class) {
					setMethod.invoke(t, Integer.valueOf(value.toString()));
				} else if (clazz == Double.class || clazz == double.class) {
					setMethod.invoke(t, Double.valueOf(value.toString()));
				} else {
					setMethod.invoke(t, value);
				}
			} catch (Exception e) {
				logger.error("赋值【{}】【{}】时发生错误！", key, value);
			}
		}
		return t;
	}
	 ...
	 * @return
	 */
	public static <K, V, E> Map<K, V> toMap(List<E> list, java.util.function.Function<E, K> kFunc, java.util.function.Function<E, V> vFunc) {
		Map<K, V> map = new HashMap<>();
		if (list == null) {
			return null;
		}
		list.forEach(e -> {
			map.put(kFunc.apply(e), vFunc.apply(e));
		});
		return map;
	}
	
	public static <T> Optional<T> safeExecute(Supplier<T> func) {
		T r = null;
		try {
			r = func.get();
		} catch (Throwable e) {
	...
	public static <T, U> void safeExecute(BiConsumer<T, U> func, T t, U u) {
		try {
			func.accept(t, u);
		} catch (Throwable e) {
			logger.warn(e.getMessage());
